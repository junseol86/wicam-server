<?php
    ...
    if ($unauthorized == 0) {
        ...
        $sql_write = "INSERT INTO wicam.restaurant (item_name, genre, description, address, latitude, longitude, phone1, phone2, school_idx,
                                                            writer_idx, writer_nickname, modifier_idx, modifier_nickname, write_time, modify_time)
                    VALUES('$restaurant_name', '$genre', '$description', '$address', '$latitude', '$longitude', '$phone1', '$phone2', '$school_id',
                            '$user_id', '$user_nickname', '$user_id', '$user_nickname', '$now', '$now')";
        $result_write = mysql_query($sql_write, $mydb);

        $id = '';
        $sql_id = "SELECT MAX(item_idx) FROM wicam.restaurant";
