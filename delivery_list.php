<?php
    ...
    $join = $see_favorite == 0 ? "" : " INNER JOIN wicam.delivery_favorite fvrt ON dlvry.item_idx = fvrt.item_idx ";
    $join_condition = $see_favorite == 0 ? "" : " AND fvrt.user_idx = $user_id ";

    $sql_count = "SELECT * FROM wicam.delivery dlvry $join
                  WHERE
                  $condition
                  $join_condition
    ...
    for ($i = 0; $i < (($unit < $to_be_added) ? $unit : $to_be_added); $i++) {
        ...
        // 전단지가 있는지 확인
        $has_photo = 0;
        if (file_exists('../image/delivery/item_'.$row['item_idx'].'.jpg'))
            $has_photo = 1;

        $sql_my_favorite = "SELECT * FROM wicam.delivery_favorite WHERE item_idx = $row[item_idx] AND user_idx = $user_id";
        $result_my_favorite = mysql_query($sql_my_favorite, $mydb);
        $my_favorite = mysql_num_rows($result_my_favorite);

        $my_like = 0;
        $sql_my_like = "SELECT * FROM wicam.delivery_like WHERE item_idx = $row[item_idx] AND user_idx = $user_id";
        ...
        $json_data['results'][$i] = array("delivery_id"=>"$row[item_idx]", "delivery_name"=>"$row[item_name]", "school_id_list"=>$school_id_array, "school_name_list"=>$school_name_array,
            "includes"=>"$row[includes]", "delivery_condition"=>"$row[delivery_condition]", "description"=>"$row[description]",
            "menu"=>"$row[menu]", "phone1"=>"$row[phone1]", "phone2"=>"$row[phone2]", "has_photo"=>"$has_photo",
