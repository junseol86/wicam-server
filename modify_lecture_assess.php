<?php
    ...
    if ($unauthorized == 0) {
        ...
        if (mysql_num_rows($result_assess_check) > 0) {
            $sql_assess_write = "UPDATE wicam.lecture_assess SET difficulty = $difficulty, instructiveness = $instructiveness, description = '$description', write_time = '$now'
                                  WHERE item_idx = $item_id AND writer_idx = $user_id";
