<?php
    ...
    if (mysql_num_rows($result_authority_check) == 0)
        ...
        $sql_writer = "SELECT writer_idx FROM wicam.delivery WHERE item_idx = $item_id";
    ...
    if ($unauthorized == 0) {
        ...
        $sql_delete = "DELETE FROM wicam.delivery WHERE item_idx = $item_id";
        $result_delete = mysql_query($sql_delete, $mydb);

        if ($result_delete == true) {

            $file_path = "../image/delivery/item_".$item_id.".jpg";
            if (file_exists($file_path))
                unlink($file_path);


            // 식당의 즐겨찾기 전부 제거
            $sql_favorite_delete = "DELETE FROM wicam.delivery_favorite WHERE item_idx = $item_id";
            mysql_query($sql_favorite_delete, $mydb);

            // 식당의 좋아요 전부 제거
            $sql_like_delete = "DELETE FROM wicam.delivery_like WHERE item_idx = $item_id";
            mysql_query($sql_like_delete, $mydb);

            // 식당의 댓글, 댓글의 사진, 댓글의 좋아요 제거
            $sql_comment_list = "SELECT item_comment_idx FROM wicam.delivery_comment WHERE item_idx = $item_id";
            ...
            if (mysql_num_rows($result_comment_list) != null) {
                for ($i = 0; $i < mysql_num_rows($result_comment_list); $i++) {
                    ...
                    if (file_exists('../image/delivery/'.$comment_id.'.jpg'))
                        unlink('../image/delivery/'.$comment_id.'.jpg');

                    $sql_comment_like_delete = "DELETE FROM wicam.delivery_comment_like WHERE item_comment_idx = $comment_id";
                    mysql_query($sql_comment_like_delete, $mydb);

                }
            }
            $sql_delete_comment = "DELETE FROM wicam.delivery_comment WHERE item_idx = $item_id";
