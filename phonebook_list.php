<?php
    ...
    $join = $see_favorite == 0 ? "" : " INNER JOIN wicam.phonebook_favorite fvrt ON dlvry.item_idx = fvrt.item_idx ";
    $join_condition = $see_favorite == 0 ? "" : " AND fvrt.user_idx = $user_id ";

    $sql_count = "SELECT * FROM wicam.phonebook dlvry $join
                  WHERE
                  $condition
                  $join_condition
    ...
    for ($i = 0; $i < (($unit < $to_be_added) ? $unit : $to_be_added); $i++) {
        ...
        // 전단지가 있는지 확인
        $has_photo = 0;
        if (file_exists('../image/phonebook/item_'.$row['item_idx'].'.jpg'))
            $has_photo = 1;

        $sql_my_favorite = "SELECT * FROM wicam.phonebook_favorite WHERE item_idx = $row[item_idx] AND user_idx = $user_id";
        $result_my_favorite = mysql_query($sql_my_favorite, $mydb);
        $my_favorite = mysql_num_rows($result_my_favorite);

        $my_like = 0;
        $sql_my_like = "SELECT * FROM wicam.phonebook_like WHERE item_idx = $row[item_idx] AND user_idx = $user_id";
        ...
        $json_data['results'][$i] = array("phonebook_id"=>"$row[item_idx]", "phonebook_name"=>"$row[item_name]", "school_id_list"=>$school_id_array, "school_name_list"=>$school_name_array,
