<?php
    ...
    if ($unauthorized == 0) {
        ...
        $sql_write = "INSERT INTO wicam.phonebook (item_name, kind, phone1, phone2, url_link, description, school_idx,
                                                            writer_idx, writer_nickname, modifier_idx, modifier_nickname, write_time, modify_time)
                    VALUES('$phonebook_name', '$kind', '$phone1', '$phone2', '$url_link', '$description', '$school_id',
                            '$user_id', '$user_nickname', '$user_id', '$user_nickname', '$now', '$now')";
        $result_write = mysql_query($sql_write, $mydb);

        $id = '';
        $sql_id = "SELECT MAX(item_idx) FROM wicam.phonebook";
