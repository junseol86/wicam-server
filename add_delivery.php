<?php
    ...
    if ($unauthorized == 0) {
        ...
        $sql_write = "INSERT INTO wicam.delivery (item_name, menu, phone1, phone2, delivery_condition, description, includes, school_idx,
                                                            writer_idx, writer_nickname, modifier_idx, modifier_nickname, write_time, modify_time)
                    VALUES('$delivery_name', '$menu', '$phone1', '$phone2', '$delivery_condition', '$description', '$includes', '$school_id',
                            '$user_id', '$user_nickname', '$user_id', '$user_nickname', '$now', '$now')";
        $result_write = mysql_query($sql_write, $mydb);

        $id = '';
        $sql_id = "SELECT MAX(item_idx) FROM wicam.delivery";
